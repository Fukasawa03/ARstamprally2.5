...
var DEFAULT_CONFIG = {
  ...
  eventYear:'2026', eventTitle:'文化祭\nスタンプラリー',
  eventSubtitle:'全スタンプを集めて特典をゲットしよう！',
  ui:{
    btnStart:'スタート！', btnHowto:'遊び方', howtoTitle:'遊び方',
    mapTitle:'スタンプマップ', progressLabel:'進捗', btnScan:'ARスキャン開始',
    arTitle:'マーカーを照らして！', arHint:'マーカーを枠内に合わせてください',
    stampAcquiredLabel:'スタンプ獲得！', btnToMap:'マップに戻る', btnShare:'シェア',
    completeTitle:'コンプリート！', completeSubtitle:'全スタンプ制覇おめでとう！',
    completeTimeLabel:'クリアタイム', manualTitle:'合言葉入力',
    manualDesc:'スタンプ地点に掲示された数字を入力してください',
  },
  coupon:{ title:'🎁 特典クーポン', body:'文化祭グッズ引換券！本部で見せてください', code:'FES-2026-COMP' },
  howtoSteps:[
...
var Config = (function(){
  ...
  function load(){
    return DB.get('config').then(function(cached){
      ...
      if(sv && sv!==_cfg.versionId){ return Promise.all([DB.del('progress'),DB.del('timerStart'),DB.del('timerElapsed')]); }
...
/* ---- 4. State (progress + timer) -------------------------- */
var State = (function(){
  var _acq={}, _start=null, _elapsed=0, _running=false, _iv=null;
  function load(){
    return Promise.all([DB.get('progress'),DB.get('timerElapsed'),DB.get('timerStart')]).then(function(v){
      if(v[0]) v[0].forEach(function(id){ _acq[id]=true; });
      _elapsed=v[1]||0;
      if(v[2]){ _elapsed+=(Date.now()-v[2]); DB.del('timerStart'); }
    });
  }
  function startTimer(){
    if(_running) return;
    _running=true; _start=Date.now();
    DB.set('timerStart',_start);
    _iv=setInterval(function(){ UI.updateTimer(_elapsed+(Date.now()-_start)); },500);
  }
  function stopTimer(){
    if(!_running) return;
    _elapsed+=(Date.now()-_start);
    _running=false; _start=null;
    clearInterval(_iv); _iv=null;
    DB.del('timerStart'); DB.set('timerElapsed',_elapsed);
  }
  function getElapsed(){ return _running&&_start ? _elapsed+(Date.now()-_start) : _elapsed; }
  ...
  function reset(){ _acq={}; _elapsed=0; _start=null; _running=false; clearInterval(_iv); _iv=null; return Promise.all([DB.del('progress'),DB.del('timerStart'),DB.del('timerElapsed')]); }
...
var UI = (function(){
  ...
  /* 画面切替: display:flex / display:none を直接操作 */
  ...
  /* モーダル: display:flex / display:none */
  ...
  function toast(msg,dur){
    ...
    clearTimeout(_tt);
    _tt=setTimeout(function(){ el.style.display='none'; },dur||2500);
  }
  ...
  function applyConfig(cfg){
    ...
    st('ui-event-year',cfg.eventYear); st('ui-event-title',cfg.eventTitle);
    st('ui-event-subtitle',cfg.eventSubtitle); st('ui-btn-start',cfg.ui.btnStart);
    st('ui-btn-howto',cfg.ui.btnHowto); st('ui-howto-title',cfg.ui.howtoTitle);
    st('ui-map-title',cfg.ui.mapTitle); st('ui-progress-label',cfg.ui.progressLabel);
    st('ui-btn-scan',cfg.ui.btnScan); st('ui-ar-title',cfg.ui.arTitle);
    st('ui-ar-hint',cfg.ui.arHint); st('ui-stamp-acquired-label',cfg.ui.stampAcquiredLabel);
    st('ui-btn-to-map',cfg.ui.btnToMap); st('ui-btn-share',cfg.ui.btnShare);
    st('ui-complete-title',cfg.ui.completeTitle); st('ui-complete-subtitle',cfg.ui.completeSubtitle);
    st('ui-complete-time-label',cfg.ui.completeTimeLabel);
    st('ui-manual-title',cfg.ui.manualTitle); st('ui-manual-desc',cfg.ui.manualDesc);
    st('ui-coupon-title',cfg.coupon.title); st('ui-coupon-body',cfg.coupon.body);
  ...
  function showStampAcquired(stamp){
    ...
    for(var i=0;i<22;i++){
      ...
      d.style.cssText='left:'+Math.random()*100+'%;top:'+(50+Math.random()*40)+'%;width:'+sz+'px;height:'+sz+'px;background:'+cols[Math.floor(Math.random()*4)]+';--tx:'+tx+'px;--ty:'+ty+'px;--d:'+(1+Math.random())+'s;--dl:'+(Math.random()*.4)+'s;';
      c.appendChild(d);
    }
  }
...
function startConfetti(){
  ...
  var pp=[]; for(var i=0;i<120;i++) pp.push({x:Math.random()*canvas.width,y:-20-Math.random()*200,r:4+Math.random()*6,c:cols[Math.floor(Math.random()*6)],sp:2+Math.random()*3,dr:(Math.random()-.5)*1.5,a:Math.random()*Math.PI*2,sp2:(Math.random()-.5)*.15});
  var fr; (function draw(){ ctx.clearRect(0,0,canvas.width,canvas.height); pp.forEach(function(p){ ctx.save(); ctx.translate(p.x,p.y); ctx.rotate(p.a); ctx.fillStyle=p.c; ctx.fillRect(-p.r/2,-p.r/2,p.r,p.r*1.7); ctx.restore(); p.y+=p.sp; p.x+=p.dr; p.a+=p.sp2; if(p.y>canvas.height){ p.y=-20; p.x=Math.random()*canvas.width; } }); fr=requestAnimationFrame(draw); }());
  setTimeout(function(){ cancelAnimationFrame(fr); ctx.clearRect(0,0,canvas.width,canvas.height); },8000);
}

/* ---- 7. Share --------------------------------------------- */
function doShare(title,text){ if(navigator.share){ navigator.share({title:title,text:text}).catch(function(){}); }else if(navigator.clipboard){ navigator.clipboard.writeText(text).then(function(){ UI.toast('クリップボードにコピーしました！'); }).catch(function(){}); } }

/* ---- 8. AR ------------------------------------------------- */
var AR = (function(){
  var _stamps=[], _onDetect=null, _cd={}, _scene=null;
  ...
  function startScene(){
    ...
    function build(){
      ...
      if(_scene&&_scene.hasLoaded) attach(); else if(_scene) _scene.addEventListener('loaded',attach);

      // カメラ起動メッセージ
      var st=document.getElementById('ar-status');
      if(st){
        st.textContent='カメラ起動中...';
        var obs=new MutationObserver(function(){ if(wrapper.querySelector('video')){ st.textContent=''; obs.disconnect(); } });
        obs.observe(wrapper,{childList:true,subtree:true});
        setTimeout(function(){ if(st.textContent==='カメラ起動中...') st.textContent='⌨️ ボタンで合言葉入力もできます'; },7000);
      }
    }
    ...
    if(typeof AFRAME==='undefined'){
      ...
      s1.onload=function(){
        ...
        s2.onload=build; document.head.appendChild(s2);
      };
  ...
  function destroyScene(){
    var wrapper=document.getElementById('ar-scene-wrapper'); if(wrapper) wrapper.innerHTML='';
    _scene=null; _cd={};
  }
...
function makeDraggable(listEl, onReorder){
  ...
  function bindAll(){
    items().forEach(function(item){
      ...
      item.ondragstart=function(e){ src=item; item.classList.add('dragging'); e.dataTransfer.effectAllowed='move'; };
...
var Admin = (function(){
  ...
  function renderAll(){
    renderStampList_(); renderMindList(); renderTextEditors(); renderHowtoEditor();
  ...
  function renderStampList_(){
    ...
    list.innerHTML=_cfg.stamps.map(function(s,i){
      return '<div class="stamp-admin-item" data-idx="'+i+'">'
        +'<span class="drag-icon" title="ドラッグで並び替え">☰</span>'
        +'<span class="stamp-emoji">'+(s.emoji||'⭐')+'</span>'
        +'<div class="stamp-admin-info"><div class="stamp-admin-name">'+UI.esc(s.name)+'</div>'
        +'<div class="stamp-admin-sub">'+(s.mindFile?'.mind':'バーコード#'+s.barcodeId)+' | 合言葉:'+s.code+'</div></div>'
    ...
    if(!_drag) _drag=makeDraggable(list,function(si,di){ var a=_cfg.stamps.slice(), m=a.splice(si,1)[0]; a.splice(di,0,m); _cfg.stamps=a; Config.save(); renderStampList_(); UI.updateProgress(_cfg.stamps); UI.toast('並び順を更新しました ✓'); });
    _drag.rebind();
  }
  ...
  function renderMindList(){
    ...
    list.querySelectorAll('[data-idx]').forEach(function(b){
      b.onclick=function(){ var idx=parseInt(b.dataset.idx), name=_cfg.mindFiles[idx].name; _cfg.mindFiles.splice(idx,1); _cfg.stamps.forEach(function(s){ if(s.mindFile===name) s.mindFile=''; }); Config.save(); renderMindList(); renderStampList_(); UI.toast('削除しました'); };
    });
  }

  function renderTextEditors(){
    renderGroup('text-editors-event',[{key:'eventYear',label:'年度'},{key:'eventTitle',label:'タイトル'},{key:'eventSubtitle',label:'サブタイトル'}]);
    renderGroup('text-editors-ui',Object.keys(_cfg.ui).map(function(k){ return {key:'ui.'+k,label:k}; }));
    renderGroup('text-editors-coupon',[{key:'coupon.title',label:'クーポンタイトル'},{key:'coupon.body',label:'クーポン本文'},{key:'coupon.code',label:'クーポンコード'}]);
  }

  function renderGroup(cid,rows){
    var c=document.getElementById(cid); if(!c) return;
    c.innerHTML=rows.map(function(r){
      var keys=r.key.split('.'), obj=_cfg; keys.forEach(function(k){ obj=obj&&obj[k]; });
      return '<div class="text-edit-row"><label>'+UI.esc(r.label)+'</label><input type="text" class="admin-input text-edit-input" data-key="'+r.key+'" value="'+UI.esc(obj||'')+'"></div>';
    }).join('');
  }

  function collectTexts(){ document.querySelectorAll('.text-edit-input').forEach(function(el){ var keys=el.dataset.key.split('.'), obj=_cfg; for(var i=0;i<keys.length-1;i++) obj=obj[keys[i]]; obj[keys[keys.length-1]]=el.value; }); }
  ...
  function deleteStamp(id){ if(!confirm('このスタンプを削除しますか？')) return; _cfg.stamps=_cfg.stamps.filter(function(s){ return s.id!==id; }); Config.save(); renderAll(); UI.hideModal('modal-stamp-edit'); UI.toast('削除しました'); AR.init(_cfg.stamps,App.onStampDetect); UI.updateProgress(_cfg.stamps); }
  ...
  function bindStatic(){
    ...
    B('btn-mind-browse',function(){ if(mi) mi.click(); });
    ...
    if(dz){
      ...
      dz.ondragover=function(e){ e.preventDefault(); dz.classList.add('drag-active'); };
      dz.ondragleave=function(){ dz.classList.remove('drag-active'); };
      dz.ondrop=function(e){ e.preventDefault(); dz.classList.remove('drag-active'); var f=e.dataTransfer.files[0]; if(f&&f.name.endsWith('.mind')) readMind(f); else UI.toast('.mindファイルのみ対応しています'); };
    }
  ...
  function showLeaderboard(){
    ...
    fetch(_cfg.leaderboardUrl+'?version='+encodeURIComponent(_cfg.versionId)).then(function(r){ return r.json(); }).then(function(d){
      var medals=['r1','r2','r3'];
      list.innerHTML=(d.entries&&d.entries.length)?d.entries.map(function(e,i){ return '<div class="lb-item"><div class="lb-rank '+(medals[i]||'')+'">'+(i+1)+'</div><div class="lb-name">'+UI.esc(e.name)+'</div><div class="lb-time">'+UI.fmtTime(e.time)+'</div></div>'; }).join(''):'<div style="text-align:center;color:var(--muted);padding:24px;">記録なし</div>';
