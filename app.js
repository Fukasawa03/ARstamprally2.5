  ...
  function showComplete(elapsed) {
    ...
    DB.get('couponUsed').then(function (used) {
      if (used) {
        ...
        var card2 = document.getElementById('coupon-card');
        var useArea2  = document.getElementById('coupon-use-area');
        var usedArea2 = document.getElementById('coupon-used-area');
        if (card2)     card2.classList.remove('used');
        if (useArea2)  useArea2.style.display = 'block';
        if (usedArea2) usedArea2.style.display = 'none';
      }
    });
...
/* ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
   ...
   問題の根本原因:
   - A-Frame 1.4 + AR.js の組み合わせで iOS Safari は
     getUserMedia の権限取得タイミングが合わずブラックアウトする
   - 解決策: 先に getUserMedia でカメラストリームを取得してから
     A-Frame シーンを構築する
   ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━ */
var AR = (function () {
  var _stamps    = [];
  var _onDetect  = null;
  var _cd        = {};
  var _stream    = null;  // カメラストリームの参照

  function init(stamps, onDetect) { _stamps = stamps; _onDetect = onDetect; }

  /* カメラ権限を先に取得 → A-Frameロード → シーン構築 */
  function startScene() {
    UI.showScreen('screen-ar');

    var msgEl = document.getElementById('ar-msg');
    var cfg   = Config.get();
    if (msgEl) msgEl.textContent = cfg && cfg.ui.arCameraMsg || 'カメラ起動中...';

    // ★ Step1: getUserMediaでカメラ権限を先取りする
    navigator.mediaDevices.getUserMedia({ video: { facingMode: 'environment' }, audio: false })
      .then(function (stream) {
        _stream = stream;
        // 権限取得できたらすぐストリームは止める (A-Frameが改めて取得する)
      ...
      .catch(function (err) {
        ...
        if (msgEl) msgEl.textContent = cfg && cfg.ui.arCameraFallback || '⌨️ ボタンで合言葉入力もできます';
        // 権限なしでも合言葉入力は使えるので画面は維持
      });
  }
  ...
  function buildScene() {
    ...
    function build() {
      ...
      // videoとcanvasのスタイルを監視して修正
      var styleObs = new MutationObserver(function () {
        document.querySelectorAll('body > video').forEach(function (v) {
        ...
        document.querySelectorAll('.a-canvas').forEach(function (c) {
      ...
      styleObs.observe(document.body, { childList: true, subtree: true, attributes: true, attributeFilter: ['style'] });
...
var Admin = (function () {
  ...
  function renderAll() {
    ...
    var vb = document.getElementById('admin-ver-badge'); if (vb) vb.textContent = 'v: ' + _cfg.versionId;
  ...
  function renderStampList_() {
    ...
    if (!_drag) {
      _drag = makeDraggable(list, function (si, di) {
        var a = _cfg.stamps.slice(), m = a.splice(si,1)[0]; a.splice(di,0,m);
        _cfg.stamps = a; Config.save(); renderStampList_();
        UI.updateProgress(_cfg.stamps); UI.toast('並び順を更新しました ✓');
      });
    }
    _drag.rebind();
  }
  ...
  function renderMindList() {
    ...
    if (!files.length) { list.innerHTML = '<p style="font-size:13px;color:var(--muted);padding:8px 0;">登録ファイルなし</p>'; return; }
    list.innerHTML = files.map(function (f, i) {
      var linked = _cfg.stamps.find(function (s) { return s.mindFile === f.name; });
      return '<div class="mind-file-item"><span style="font-size:20px">📄</span>'
        + '<div class="mind-file-name">'+UI.esc(f.name)+'</div>'
        + '<div class="mind-file-linked">'+(linked?'→ '+UI.esc(linked.name):'未リンク')+'</div>'
        + '<button style="color:var(--red);font-size:18px;padding:2px 6px" data-idx="'+i+'">🗑</button></div>';
    }).join('');
    list.querySelectorAll('[data-idx]').forEach(function (b) {
      b.onclick = function () {
        var idx=parseInt(b.dataset.idx), name=_cfg.mindFiles[idx].name;
        _cfg.mindFiles.splice(idx,1);
        _cfg.stamps.forEach(function (s) { if(s.mindFile===name) s.mindFile=''; });
        Config.save(); renderMindList(); renderStampList_(); UI.toast('削除しました');
      };
    });
  }

  function renderTextEditors() {
    renderGroup('te-event',[{key:'eventYear',label:'年度（バッジに表示）'},{key:'eventTitle',label:'タイトル（改行は\\nと入力）'},{key:'eventSubtitle',label:'サブタイトル'}]);
    renderGroup('te-start',[{key:'startConfirmTitle',label:'確認ダイアログ タイトル'},{key:'startConfirmDesc',label:'確認ダイアログ 説明文'},{key:'startConfirmOk',label:'「スタート！」ボタン'},{key:'startConfirmCancel',label:'「キャンセル」ボタン'},{key:'ui.btnStart',label:'タイトル画面「スタート！」ボタン'},{key:'ui.btnHowto',label:'タイトル画面「遊び方」ボタン'}]);
    renderGroup('te-map',[{key:'ui.mapTitle',label:'マップ画面 タイトル'},{key:'ui.progressLabel',label:'進捗ラベル'},{key:'ui.btnScan',label:'「ARスキャン開始」ボタン'}]);
    renderGroup('te-ar',[{key:'ui.arTitle',label:'ARスキャン画面 タイトル'},{key:'ui.arHint',label:'スキャンヒント文'},{key:'ui.arCameraMsg',label:'カメラ起動中メッセージ'},{key:'ui.arCameraFallback',label:'カメラ失敗時メッセージ'}]);
    renderGroup('te-stamp',[{key:'ui.stampNewTag',label:'スタンプ新規獲得 タグ'},{key:'ui.stampAlreadyTag',label:'スタンプ獲得済み タグ'},{key:'ui.btnStampBack',label:'スタンプ画面「マップに戻る」ボタン'},{key:'ui.manualTitle',label:'合言葉モーダル タイトル'},{key:'ui.manualDesc',label:'合言葉モーダル 説明文'}]);
    renderGroup('te-complete',[{key:'ui.completeTitle',label:'コンプリート タイトル'},{key:'ui.completeSubtitle',label:'コンプリート サブタイトル'},{key:'ui.completeTimeLabel',label:'クリアタイム ラベル'},{key:'ui.btnCompleteMap',label:'「スタンプ一覧を見る」ボタン'}]);
    renderGroup('te-coupon',[{key:'coupon.title',label:'クーポン タイトル'},{key:'coupon.body',label:'クーポン 本文'},{key:'coupon.code',label:'クーポン コード'}]);
  }
  ...
  function renderHowtoEditor() {
    ...
    c.querySelectorAll('input').forEach(function (el) {
      el.oninput = function () { var idx=parseInt(el.dataset.idx); if(_cfg.howtoSteps[idx]) _cfg.howtoSteps[idx][el.dataset.field]=el.value; };
    });
  ...
  function openEdit(id) {
    var s=id?(_cfg.stamps.find(function(x){return x.id===id;})||{}):{}, isNew=!s.id;
  ...
  function saveEdit() {
    ...
    var data = {
      id: id||('s'+Date.now()), name: name, location: loc,
  ...
  function readMind(file) {
    ...
    r.onload = function (e) {
      ...
      Config.save(); renderMindList(); UI.toast('.mindファイルを追加しました ✓');
    };
  ...
  function showLeaderboard() {
    ...
    fetch(_cfg.leaderboardUrl+'?version='+encodeURIComponent(_cfg.versionId))
      ...
      .catch(function(){ if(list) list.innerHTML='<p style="text-align:center;color:var(--muted);padding:24px">読み込みに失敗しました</p>'; });
  }
  ...
  function bindStatic() {
    ...
    if (dz) {
      ...
      dz.ondrop=function(e){e.preventDefault();dz.classList.remove('drag-over');var f=e.dataTransfer.files[0];if(f&&f.name.endsWith('.mind'))readMind(f);else UI.toast('.mindファイルのみ対応しています');};
    }
    ...
    B('btn-update-version', function(){
      ...
      Config.save().then(function(){return DB.set('versionId',nv);})
        ...
        .then(function(){
          UI.updateProgress(_cfg.stamps); UI.updateTimer(0);
          var vb=document.getElementById('admin-ver-badge'); if(vb) vb.textContent='v: '+nv;
    ...
    B('btn-reset-self', function(){
      ...
      State.reset().then(function(){
        ...
        if (btn) {
          ...
          btn.style.background=''; btn.style.boxShadow=''; btn.classList.add('btn-glow');
        }
    ...
    B('btn-fetch-sheets', function(){
      ...
      Config.fetchSheets(url).then(function(){
        Config.save(); renderAll(); UI.applyConfig(_cfg); UI.updateProgress(_cfg.stamps);
        if (st) st.textContent='✓ 取得成功'; UI.toast('読み込みました ✓');
      }).catch(function(e){ if(st) st.textContent='✗ 失敗: '+e.message; UI.toast('取得に失敗しました'); });
    });
    ...
    B('btn-export-json', function(){
      ...
      a.download='stamp-rally-'+_cfg.versionId+'.json'; a.click();
    });
      ...
      imp.onchange=function(e){
        ...
        r.onload=function(ev){
          try {
            ...
            Config.save(); renderAll(); UI.applyConfig(_cfg); UI.updateProgress(_cfg.stamps);
            AR.init(_cfg.stamps,App.onStampDetect); UI.toast('インポートしました ✓');
...
var App = {
  ...
  bindNav: function () {
    ...
    B('btn-ar-manual', function () {
      var inp=document.getElementById('manual-input'), err=document.getElementById('manual-err');
    ...
    B('btn-admin-entry', function () {
      ...
      else if (pw !== null) { UI.toast('パスワードが違います'); }
    });
  ...
  bindModals: function () {
    ...
    if (sl) {
      sl.onclick = function (e) {
        ...
        if (!stamp) return;
        App._curStamp = stamp;
        if (State.isAcquired(id)) UI.showStampDetail(stamp, false);
      };
    }
  },
};
