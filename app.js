...
var AR = (function () {
  ...
  function buildMindAR(arStamps) {
    ...
    // canvasを透明に（黒い点対策）
    // MindARのrendererはalpha:falseで生成されるため、
    // videoを直接canvasに描画している部分の外側が黒くなる。
    // videoのz-indexとcanvasのz-indexを正しく設定して対処する。
    ...
    _mindar.start()
      .then(function () {
        ...
        // videoをcanvasの下(z-index:0)で画面全体にcover表示
        // MindARはvideoをleft:-64pxなどずらして配置するため強制上書き
        var vid = container.querySelector('video');
        if (vid) {
          vid.style.cssText =
            'position:fixed!important;top:0!important;left:0!important;'
