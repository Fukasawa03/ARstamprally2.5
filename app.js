...
/* ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
   7. AR (MindAR v1.1.x + Three.js — script tag方式)

   v1.2.0以降はES Moduleのみ対応で window.MINDAR に入らない。
   v1.1.x は <script src> で読み込め window.MINDAR.IMAGE.MindARThree が使える。
   
   使用バージョン:
   - mind-ar@1.1.5 (mindar-image.prod.js) — Three.jsを内包
   - Three.jsは mind-ar@1.1.5 に同梱されている (window.MINDAR.IMAGE.THREE)
   ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━ */
var AR = (function () {
  var _stamps      = [];
  var _onDetect    = null;
  ...
  function startScene() {
    ...
    // MindAR v1.1.5: Three.js内包版。unpkgはnpmと直接同期で安定している。
    // jsdelivr/gh (GitHub CDN) はキャッシュが不安定なためunpkgを使う。
  ...
  function buildMindAR(arStamps) {
    ...
    // MindARが自分でvideoとcanvasを生成するため、containerは空にする
    container.innerHTML = '';

    // .mindファイルをBlob URLに変換
    ...
    try {
      ...
      if (!base64) throw new Error('base64 data missing');
    ...
    if (!window.MINDAR || !window.MINDAR.IMAGE || !window.MINDAR.IMAGE.MindARThree) {
      ...
      console.error('window.MINDAR.IMAGE.MindARThree not found');
      return;
    }

    // MindARThree 初期化（v1.1.x APIに準拠）
    try {
      _mindar = new window.MINDAR.IMAGE.MindARThree({
        ...
        uiError:        'no',
        // --- 検出速度チューニング ---
        // filterMinCF/filterBeta: トラッキングの滑らかさ調整
        // 小さいほど滑らか、大きいほど追従が速い
        filterMinCF:    0.001,
        filterBeta:     1000,
        // missTolerance: 何フレームマーカーを見失ったら消すか（デフォルト5）
        // 小さくすると誤検出が減る、大きくすると安定する
        missTolerance:  3,
        // warmupTolerance: 何フレーム連続検出したら確定とするか（デフォルト5）
        // 小さくすると速く反応する（1が最速）
        warmupTolerance: 1,
      });
    ...
    // THREE は mind-ar v1.1.x に同梱されている
    ...
    arStamps.forEach(function (stamp, idx) {
      ...
      anchor.onTargetFound = function () { handleDetect(stamp); };

      // マーカー検出時の視覚フィードバック（光るプレーン）
      var geo  = new THREE.PlaneGeometry(1, 1);
      var mat  = new THREE.MeshBasicMaterial({
        color: 0xc840ff, transparent: true, opacity: 0.4, side: THREE.DoubleSide,
      });
      anchor.group.add(new THREE.Mesh(geo, mat));
    });

    // start() でカメラ権限ダイアログが出て映像が開始される
    // iOSはHTTPS必須。GitHub Pages等のHTTPS環境で動作する。
    _mindar.start()
      .then(function () {
        setMsg('');
        // v1.1.5 Three.js版は update() 不要。render()のみ。
        renderer.setAnimationLoop(function () {
          renderer.render(scene, camera);
        });

        // MindARが生成したvideoを全画面・高画質に強制設定
        // ライブラリがvideoのスタイルを上書きするので start()後に設定する
        var videos = container.querySelectorAll('video');
        videos.forEach(function (v) {
          v.style.cssText = 'position:absolute!important;top:0!important;left:0!important;'
        ...
        canvases.forEach(function (c) {
          c.style.cssText = 'position:absolute!important;top:0!important;left:0!important;'
            + 'width:100%!important;height:100%!important;z-index:1!important;';
        });
      })
      .catch(function (e) {
        console.error('MindAR start error:', e.name, e.message, e);
        var cfg2 = Config.get();
        var msg = (cfg2 && cfg2.ui.arCameraFallback) || 'カメラを起動できませんでした';
  ...
  function destroyScene() {
    ...
    // Blob URLを解放
    if (_mindBlobUrl) {
      try { URL.revokeObjectURL(_mindBlobUrl); } catch(e) {}
      _mindBlobUrl = null;
    }
    var container = document.getElementById('ar-container');
