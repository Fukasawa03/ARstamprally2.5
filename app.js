'use strict';
/* ============================================================
   WebAR Stamp Rally — app.js  (カメラ・UI修正版)
   ============================================================ */
...
var AR = (function () {
  ...
  function startScene() {
    ...
    function build() {
      ...
      // ★ 修正: a-sceneにstyle属性を付けてz-indexを確実に設定
      //   embedded属性でA-Frameが独自のキャンバスをbody直下ではなくa-scene内に配置するよう促す
      //   ただしAR.jsはvideoをbodyに追加するため、CSSで対処
      wrapper.innerHTML =
        '<a-scene embedded '
      ...
      // ★ 修正: A-Frameのvideoとcanvasにz-indexを強制適用するMutationObserver
      var videoObserver = new MutationObserver(function () {
        // bodyに追加されたvideo要素を修正
      ...
      videoObserver.observe(document.body, { childList: true, subtree: true, attributes: true, attributeFilter: ['style'] });
