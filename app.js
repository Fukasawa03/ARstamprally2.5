...
/* ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
   7. AR (MindAR.js 単体 — A-Frame不使用)
   
   iOS/Android両対応の理由:
   - MindARはWebGL + getUserMediaを直接制御する
   - A-Frameを介さないため iOS Safariの干渉がない
   - mindar-image.prod.js 単体で動作する
   ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━ */
var AR = (function () {
  var _stamps   = [];
  var _onDetect = null;
  var _cd       = {};       // クールダウン
  var _mindar   = null;     // MindARインスタンス
  var _loaded   = false;
  ...
  function startScene() {
    ...
    if (arStamps.length === 0) {
      if (msgEl) msgEl.textContent = (cfg && cfg.ui.arCameraFallback) || '合言葉入力をご利用ください';
      return;
    }

    // MindAR CDN読み込み（初回のみ）
    var cdnUrl = 'https://cdn.jsdelivr.net/npm/mind-ar@1.2.5/dist/mindar-image.prod.js';
    loadScript(cdnUrl).then(function () {
      if (msgEl) msgEl.textContent = '';
      buildMindAR(arStamps);
    }).catch(function (e) {
      console.error('MindAR load error:', e);
      if (msgEl) msgEl.textContent = (cfg && cfg.ui.arCameraFallback) || 'カメラを起動できませんでした';
    });
  }
  ...
  function buildMindAR(arStamps) {
    ...
    // 使用するmindファイル (複数スタンプが同じmindを使う可能性があるため重複排除)
    // ※ MindARは1インスタンスに1つのmindファイル。
    //   複数マーカーを使うにはすべてのマーカーを含む1つの.mindファイルを作成する必要がある。
    //   ここではstamps[0]のmindFileDataを使い、インデックスで各スタンプと対応させる。
    var mindData = arStamps[0].mindFileData;

    // Blob URLを作成
    var mindBlob;
    try {
      var byteStr = atob(mindData.split(',')[1]);
      var arr = new Uint8Array(byteStr.length);
      for (var i = 0; i < byteStr.length; i++) arr[i] = byteStr.charCodeAt(i);
      mindBlob = URL.createObjectURL(new Blob([arr], { type: 'application/octet-stream' }));
    } catch (e) {
      console.error('mindFile decode error:', e);
      var msgEl = document.getElementById('ar-msg');
      if (msgEl) msgEl.textContent = '.mindファイルの読み込みに失敗しました';
      return;
    }

    // video要素を作成（MindARが使う）
    ...
    try {
      ...
      var msgEl2 = document.getElementById('ar-msg');
      if (msgEl2) msgEl2.textContent = 'AR初期化に失敗しました';
      return;
    }
    ...
    arStamps.forEach(function (stamp, idx) {
      ...
      // 3Dオブジェクトを追加（Three.js）
      if (typeof THREE !== 'undefined') {
        var geometry = new THREE.BoxGeometry(0.1, 0.1, 0.1);
        var material = new THREE.MeshBasicMaterial({ color: 0xc840ff, transparent: true, opacity: 0.8 });
    ...
    // AR開始
    _mindar.start().then(function () {
      var msgEl3 = document.getElementById('ar-msg');
      ...
      var cfg = Config.get();
      var msgEl4 = document.getElementById('ar-msg');
      if (msgEl4) msgEl4.textContent = (cfg && cfg.ui.arCameraFallback) || 'カメラを起動できませんでした';
      URL.revokeObjectURL(mindBlob);
    });
  }
  ...
  function destroyScene() {
    if (_mindar) {
      try {
        _mindar.renderer.setAnimationLoop(null);
        _mindar.stop();
      } catch(e) {}
      _mindar = null;
    }
    // body内のvideo要素をすべて停止・削除
