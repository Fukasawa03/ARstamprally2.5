...
/* ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
   7. AR (MindAR.js + Three.js — A-Frame完全不使用)

   正しい初期化順序:
   1. Three.js を読み込む
   2. MindAR (mindar-image.prod.js) を読み込む
   3. MindARThree インスタンスを生成
   4. mindar.start() でカメラ権限取得 + 映像開始
   
   注意: container は空でなければならない（MindARが自分でvideoとcanvasを生成する）
   ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━ */
var AR = (function () {
  var _stamps   = [];
  var _onDetect = null;
  ...
  function startScene() {
    ...
    // .mindファイルが設定されているスタンプだけ対象
    var arStamps = (_stamps || []).filter(function (s) { return s.mindFile && s.mindFileData; });

    if (arStamps.length === 0) {
      setMsg((cfg && cfg.ui.arCameraFallback) || '合言葉入力をご利用ください');
      return;
    }

    // Three.js → MindAR の順で読み込む（これが必須）
    var THREE_CDN  = 'https://cdnjs.cloudflare.com/ajax/libs/three.js/r128/three.min.js';
    var MINDAR_CDN = 'https://cdn.jsdelivr.net/npm/mind-ar@1.2.5/dist/mindar-image.prod.js';

    loadScript(THREE_CDN)
      .then(function () { return loadScript(MINDAR_CDN); })
      .then(function () {
        setMsg('');
      ...
      .catch(function (e) {
        ...
        var cfg2 = Config.get();
        setMsg((cfg2 && cfg2.ui.arCameraFallback) || 'ライブラリの読み込みに失敗しました');
      });
  }
  ...
  function buildMindAR(arStamps) {
    ...
    // ★ containerは完全に空にする（MindARが自前でvideoとcanvasを生成する）
    container.innerHTML = '';

    // .mindファイルをBlob URLに変換
    // 複数スタンプは1つの.mindファイルにすべてのマーカーを含める運用
    // （MindAR Compilerで複数画像を1ファイルにまとめ、インデックスで対応させる）
    ...
    // MindARThree 初期化
    // ★ uiLoading/uiScanning/uiError は boolean true/false（文字列'no'はNG）
    var mindarInstance;
    try {
      mindarInstance = new window.MINDAR.IMAGE.MindARThree({
        container:        container,
        imageTargetSrc:   _mindBlobUrl,
        maxTrack:         arStamps.length,
        uiLoading:        true,   // MindAR標準ローディングUI
        uiScanning:       false,  // 独自スキャンUIを使うのでOFF
        uiError:          true,
      });
    } catch (e) {
      console.error('MindAR init error:', e);
      setMsg('AR初期化に失敗しました。カメラの権限を確認してください。');
      return;
    }

    _mindar = mindarInstance;

    var renderer = _mindar.renderer;
    var scene    = _mindar.scene;
    var camera   = _mindar.camera;

    // ★ renderer の domElement（canvas）のスタイルをARに合わせる
    //    MindAR が container に追加した後にスタイル上書き
    renderer.setSize(container.clientWidth || window.innerWidth, container.clientHeight || window.innerHeight);
    renderer.domElement.style.cssText =
      'position:absolute!important;top:0!important;left:0!important;'
      + 'width:100%!important;height:100%!important;z-index:2!important;';

    // 各スタンプにアンカーを設定（インデックスは.mindファイル内のマーカー順と一致させる）
    arStamps.forEach(function (stamp, idx) {
      var anchor = _mindar.addAnchor(idx);

      // ★ onTargetFound / onTargetLost は MindAR 1.2.x の正しいコールバック名
      anchor.onTargetFound = function () { handleDetect(stamp); };

      // Three.jsオブジェクトをアンカーに追加（視覚的フィードバック）
      var geo  = new THREE.PlaneGeometry(1, 1);
      var mat  = new THREE.MeshBasicMaterial({
        color: 0xc840ff, transparent: true, opacity: 0.45, side: THREE.DoubleSide,
      });
      var mesh = new THREE.Mesh(geo, mat);
      anchor.group.add(mesh);
    });

    // カメラ起動 & AR開始（ここで getUserMedia が呼ばれる）
    _mindar.start()
      .then(function () {
        setMsg('');
        // レンダーループ
      ...
      .catch(function (e) {
        ...
        var cfg3 = Config.get();
        setMsg((cfg3 && cfg3.ui.arCameraFallback) || 'カメラを起動できませんでした。権限を確認してください。');
      });
  }
