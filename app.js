...
var AR = (function () {
  ...
  function startScene() {
    ...
    // MindAR v1.1.5 は Three.js を内包している。
    // mindar-image.prod.js だけ読み込めば window.MINDAR.IMAGE.MindARThree が使える。
    var MINDAR_URL = 'https://cdn.jsdelivr.net/gh/hiukim/mind-ar-js@1.1.5/dist/mindar-image.prod.js';

    var loadPromise = _scriptsLoaded
      ? Promise.resolve()
      : loadScript(MINDAR_URL).then(function () { _scriptsLoaded = true; });
  ...
  function buildMindAR(arStamps) {
    ...
    _mindar.start()
      ...
      .catch(function (e) {
        ...
        var cfg = Config.get();
        setMsg((cfg && cfg.ui.arCameraFallback) || 'カメラを起動できませんでした');
      });
  }
